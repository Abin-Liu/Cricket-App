...
IMPLEMENT_DYNAMIC(CMainFrame, CGameFrameWnd)
...
HWND CMainFrame::OnRequestGameWnd() const
{
	return AfxGetWowWindow();
}
...
void CMainFrame::OnConfig() 
{
	...
	m_thread.Stop();		
	CConfigDlg dlg(m_thread.GetMainKey(), m_thread.GetInteractKey());
	if (DoModal(&dlg))
	{		
		m_thread.SetMainKey(dlg.m_dwMainKey);
		m_thread.SetInteractKey(dlg.m_dwInteractKey);
	}	
}
